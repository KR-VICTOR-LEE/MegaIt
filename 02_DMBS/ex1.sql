...
2. TO_DATE 단일행함수를 이용하여 EMP 테이블에서 HIREDATE가 1981년 2월 20일부터 1981년 5월 1일 사이에 
입사한 사원의 사원명, 직책, 입사일을 최초입사일부터 최근입사일 순으로 정렬하여 출력하는 SQL문을 작성하시오(5점).
...
사원명, 급여, 부서번호, 부서명, 근무지, 직속상사명을 상사가 없는 직원까지 전체 직원 다 출력하는 SQL문을 작성하시오. 
단, 상사가 없는 경우 상사명 대신 ‘없음’을 출력한다(5점).
SELECT W.ENAME, W.SAL, W.DEPTNO, DNAME, LOC, NVL(M.ENAME, '없음') M
    FROM EMP W, EMP M, DEPT D
        WHERE W.DEPTNO=D.DEPTNO AND W.MGR=M.EMPNO(+);
4. SALGRADE 테이블의 LOSAL을 등급최저 급여, HISAL을 등급 최고 급여로 이용하여, 
EMP 테이블의 사원을 사번, 사원명, 직책, 급여, 급여등급을 급여 기준 
내림차순으로 정렬하여 출력하는 SQL문을 작성하시오(5점).
SELECT EMPNO, ENAME, JOB, SAL, GRADE
    FROM EMP, SALGRADE
    WHERE SAL BETWEEN LOSAL AND HISAL ORDER BY SAL DESC;

5. EMP 테이블에서 부서번호가 20번인 사원의 사번,
사원명, 급여를 출력하되 급여 앞에는 $를 붙이고, 숫자는 3자리마다 콤마( , )를 추가하여 출력하는 SQL문을 작성하시오.
...
직잭명, 평균급여, 금여의 합을 출력하는 SQL문을 작성하시오(5점).
SELECT JOB, AVG(SAL), SUM(SAL)
    FROM EMP
    GROUP BY JOB
    HAVING AVG(SAL) >=3000;
7. EMP테이블에서 사원명이 ‘FORD’인 직원과 직책(JOB)과 급여(SAL)이 
같은 사원의 모든 정보를 출력하는 SQL문을 작성하시오(5점).
SELECT * FROM EMP
